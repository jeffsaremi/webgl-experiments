...
async function convIm2Col(input, inputShape, kernel, kernelShape, bias, autoPad, dilations, group, pads, strides) {
    ...
    const kTD = createTextureData2(gl, 4, gl.FLOAT, adjustedKernelShape, newKernelData);
    const bTD = (bias) ? createTextureData(gl, 1, gl.FLOAT, [bias.length], bias) : null;
    const outputTD = createTextureData2(gl, 1, gl.FLOAT, outputShape, null);
    const im2colDims = calcIm2ColDims(inputShape, kernelShape, outputShape, 4);
    const im2colTD = createTextureData2(gl, 4, gl.FLOAT, im2colDims, null, im2colDims[3], im2colDims[0]*im2colDims[1]*im2colDims[2]);
