function createMatmul2DBlocksProgram(gl, readSize) {
  ...
  void main()
  {
    ...
    for(int k = 0; k < ${readSize}; ++k) {
      float a = texelFetch(A, ivec2(kOffset+k, xy.y+hOffset), 0).r;
      float b = texelFetch(B, ivec2(xy.x, kOffset+k), 0).r;
      sum += a*b;
    }
    TexelValue = vec4(sum);
  }`;
...
async function runMatmul2DBlocks(gl, program, texA, texB, width, height, sharedDim, texC, blockSize, readSize) {
  ...
  const hKOffset = gl.getUniformLocation(program, 'kOffset');
...
async function main() {
  ...
  for(let i = 0; i < testDatas.length; ++i) {
    ...
    for(let j = 0; j < blockSizes.length; ++j) {
      ...
      const readSizes = [8, 16, 32];
      for(let k = 0; k< readSizes.length; ++k) {
        const readSize = readSizes[k];
        ...
        const program = createMatmul2DBlocksProgram(gl, readSize);

        console.log(`matmul-2DBlocks blocksize: ${blockSize}, readSize: ${readSize}`);
        console.time('matmul-2DBlocks');
        await runMatmul2DBlocks(gl, program, texA, texB, width, height, sharedDim, texC, blockSize, readSize);
        console.timeEnd('matmul-2DBlocks');
